...
class Jwt_Auth_Public
{
    ...
    public function __construct($plugin_name, $version)
    {
        ...
        $this->namespace = 'hangtuahapi';
        // $this->loadDataHelper();
    }

    // public function loadDataHelper()
    // {
    //     require_once plugin_dir_path(dirname(__FILE__)) . 'public/data-helper.php';
    // }
    /**
    ...
    {
        register_rest_route($this->namespace, 'getToken', array(
            'methods' => 'POST',
            'callback' => array($this, 'generate_token'),
        ));

        register_rest_route($this->namespace, 'getPosts', array(
            'methods' => 'GET',
            'callback' => array($this, 'get_posts'),
        ));

        register_rest_route($this->namespace, 'validateToken', array(
            'methods' => 'GET',
        ...
        register_rest_route($this->namespace, 'adminVerify', array(
            'methods' => 'GET',
    ...
    public function logout(){
        try {
            wp_logout();
            return $this->create_secure_response("logout successfully");
        } catch (\Throwable $th) {
            return $this->create_secure_response("logout failed");
        }
    }

    public function get_users()
    {
        $verify = $this->adminVerify();
        if( $verify['status'] == 'verified'){
            $db = new wpdb(DB_USER, DB_PASSWORD, DB_NAME, DB_HOST);
            $data = $db->get_results('select * from sdhangtuah7_users');
            return $this->create_secure_response(
                $data
            );
        }else{
            return $verify;
    ...
    public function adminVerify() {
        // $validator = $this->validate_token(true);
        // $data = $validator['data']['userData'];
        // return $data->roles;
        $validator = $this->validate_token(true);
        $data = $validator['data']['userData'];
        ...
        if(in_array('administrator', $data->roles)){
            ...
            return array(
                'status' => 'You are Not Administrator'
            ); 
    }

    public function create_secure_response($data)
    {
        $token_valid = $this->validate_token(true);
    ...
    public function determine_current_user($user)
    {
         ...
         */
        $validate_uri = strpos($_SERVER['REQUEST_URI'], 'validateToken');
        if ($validate_uri > 0) {
            return $user;
        }
    ...
    public function validate_token($output = true)
    {
        ...
        try {
            ...
            $userData = get_user_by('id',$token->data->user->id);
            return array(
